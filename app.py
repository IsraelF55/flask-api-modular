...
app = Flask(__name__)
client = pymongo.MongoClient("mongodb+srv://modular:modular@cluster0.970tq.mongodb.net/myFirstDatabase?retryWrites=true&w=majority")
...
def recomendations():
  ...
  print('\n\nCheck ---\n', check)
  Game_seen_by_user = check.columns[check[check.index == '618e7e4e265450092372958f'].notna().any()].tolist()
  print('\n\nGame_seen_by_user  ---\n', Game_seen_by_user)
  ...
  def recomendation_kubnn( user ):
    ...
    data = pd.DataFrame({'game': Games_under_consideration, 'score': score})
    top_recomendations = data.sort_values(by='score', ascending=False).head(2)
    # print('\n\n\n top 5 pd- \n\n', top_recomendations)
